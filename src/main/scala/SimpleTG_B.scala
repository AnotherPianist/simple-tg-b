...
object SimpleTG_B extends Serializable {
  def main(args: Array[String]): Unit = {
    ...
    val path = s"$rootPath/simple-tg-b/scale$scale/${System.currentTimeMillis().toString}"
    val machines = if (args.length > 2) args(2).toInt else 8
    val ratio = if (args.length > 3) args(3).toInt else 16
    ...
    val conf = new SparkConf().setAppName("SimpleTG-B")
  ...
  implicit class RangePartitionFromDegreeRDD(self: RDD[(Long, Double)]) extends Serializable {
    def rangePartition(numMachines: Int, numVertices: Long, numEdges: Long): RDD[Long] = {
      ...
      val range2finalize = range2.map { case ((first, second), i) => if (i == range2.length - 1) ((first, numVertices - 1), i) else ((first, second), i) }
      ...
      val rangeRDD2 = rangeRDD.flatMap {
        case (first, second) =>
          val end = math.ceil((second - first + 1).toDouble / threshold.toDouble).toInt
          if (end == 1) Iterable((first, second))
          else {
            val array = new Array[(Long, Long)](end)
            var i = 0
            while (i + 1 < end) {
              array(i) = (first + threshold * i, first + threshold * (i + 1) - 1)
              i += 1
            }
            array(end - 1) = (first + threshold * (end - 1), second)
            array
          }
